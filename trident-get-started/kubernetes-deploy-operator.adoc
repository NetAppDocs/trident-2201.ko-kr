...
가장 먼저 해야 할 일은 Linux 호스트에 로그인하여 해당 호스트가 관리 중인 _ 작업 _ 을(를) 관리하고 있는지 확인하는 것입니다. link:../trident-get-started/requirements.html["지원되는 Kubernetes 클러스터"^] 에 필요한 권한이 있어야 합니다.
